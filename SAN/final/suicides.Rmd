...
## H1 There is no strong linear reletionship between year and gdp
In previous section we spoke about that there maybe no use in predicting suicide rate by countries GDP since it grows with time. Here we will check if there is indeed a linear relation between these two variables.

$H_0$: There is no linear relationshpi between year and gdp.
$H_a$: Th

```{r}
data_gdp_year <- data %>% group_by(country, year)%>%select(country, year, gdp_per_capita)%>%summarise(gdp_per_capita=mean(gdp_per_capita))
ggplot(data_gdp_year, aes(x=year, y=gdp_per_capita, col=country))+
  geom_point()+
  geom_line()+
  theme(legend.position = 'none')
fit <- lm(gdp_per_capita ~ year, data_gdp_year)
summary(fit)
ggplot(data_gdp_year, aes(x=year, y=gdp_per_capita))+
  geom_point()+
  geom_line(data = fortify(fit), aes(x = year, y = .fitted))+
  theme(legend.position = 'none')
```
