...
group_by(data, year, sex) %>% summarise(suicides_no = sum(suicides_no)/sum(population)*100000) %>% spread(sex, suicides_no) %>% mutate(prop = Male/(Male + Female))%>%
  ggplot(aes(x=year, y=prop))+
    geom_line()+
    geom_point()+
    geom_hline(aes(yintercept = mean(prop)),  linetype='dashed')+
    labs(y='Proportion of male suicide')
...
Let's start by ploting a graph of suicide development by continents w.r.t to population of a whole continent.
...
data %>% group_by(country, year, continent) %>% summarise(suicides_no = sum(suicides_no), population = sum(population))%>%
  group_by(country, continent) %>% summarise(suicide_rate = mean(suicides_no/population * 100000))%>%
  ggplot(aes(x=continent, y=suicide_rate, fill=continent))+
    geom_boxplot()+
    theme(legend.position = 'none')+
    labs(x='Continent', y='Suicides per 100k people')

data %>% group_by(country, year, sex, continent) %>% summarise(suicides_no = sum(suicides_no), population = sum(population))%>%
  group_by(country, sex, continent) %>% summarise(suicide_rate = mean(suicides_no/population * 100000))%>%
  ggplot(aes(x=continent, y=suicide_rate, fill=sex))+
    geom_boxplot()+
    labs(x='Continent', y='Suicides per 100k people')

data %>% group_by(country, year, age, continent) %>% summarise(suicides_no = sum(suicides_no), population = sum(population))%>%
  group_by(country, age, continent) %>% summarise(suicide_rate = mean(suicides_no/population * 100000))%>%
  ggplot(aes(x=continent, y=suicide_rate, fill=age))+
    geom_boxplot()+
    labs(x='Continent', y='Suicides per 100k people')
...
```{r}
data_suicide_gdp <-group_by(data, year, country, gdp_per_capita, continent) %>% summarise(suicide_rate = sum(suicides_no)/sum(population)*100000)%>%group_by(country, continent)%>%summarise(mdn_suicide_rate= mean(suicide_rate), mdn_gdp_per_capita = mean(gdp_per_capita))

data_suicide_gdp%>%
  ggplot(aes(x=mdn_gdp_per_capita, y=mdn_suicide_rate, colour=continent))+
    geom_point()+
    labs(x='Average GDP per capita', y='Suicide rate per 100k', colour='Continent')

```
```{r}
box_data <- boxplot(data_suicide_gdp$mdn_gdp_per_capita)
box_data$stats
```

```{r}
data_suicide_gdp <- data_suicide_gdp %>% mutate(income_level = case_when(mdn_gdp_per_capita < 4006.4643 ~ 'Low', 
                                                              mdn_gdp_per_capita>=4006.4643 & mdn_gdp_per_capita < 8992.6190 ~ 'Lower-middle',
                                                              mdn_gdp_per_capita>=8992.6190 & mdn_gdp_per_capita <27819.0000 ~ 'Higher-middle',
                                                              mdn_gdp_per_capita >=27819.0000 ~ 'High')) %>% 
  mutate(income_level = factor(income_level, ordered=TRUE, levels = c('Low','Lower-middle','Higher-middle', 'High')))


data_suicide_gdp%>%
  ggplot(aes(x=income_level, y=mdn_suicide_rate, fill=income_level))+
    geom_boxplot()+
...
group_by(data_income_level, year, income_level) %>% summarise(suicides_per_100k = sum(suicides_no)/sum(population)*100000) %>%
  ggplot(aes(x=year, y=suicides_per_100k, col=income_level))+
...
group_by(data_income_level, year, income_level, continent) %>% summarise(suicides_per_100k = sum(suicides_no)/sum(population)*100000) %>%
...
## H1 There is an increase in suicide rate ba age
...
In the previous section we saw that there is a difference in suice rates for continent and by age group. We saw that suicide rate among children was the lowest between all age categories. The hypothesis that we want to test is that there is no difference in suicide rate among children across continents, formaly:
* $\H_0$: There is no difference between continents.
* $\H_a$: At least one continent has different suicide rate among children.

First thing to say is that we will note consider Africa in this test. Let's plot the data

```{r}
data_h2 <- group_by(data, year, age, continent) %>% filter(age=='5-14' & continent != 'Africa') %>% summarise(suicides_per_100 = sum(suicides_no)/sum(population)*100000)
data_h2_2 <- group_by(data, country, age, continent) %>% filter(age=='5-14' & continent != 'Africa') %>% summarise(suicides_per_100 = sum(suicides_no)/sum(population)*100000)
ggplot(data_h2_2, aes(x=continent, y=suicides_per_100, fill=age))+
  geom_boxplot()+
  labs(x='Continent', y='Suicides per 100k people')+
  theme(legend.position = 'none')
...
To test this hypothesis we will use ANOVA. ANOVA has several inportant assumtions:

1. Samples are drawn independantly of each other
2. Samples from each groups are drawn from normal distribution
3. All groups have common variance
...
## H3 There is a difference in suicide rate by income level
