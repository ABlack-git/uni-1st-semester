...
Isomap <- function(in_data, k=5){
  knns<-get.knn(in_data, k = k)
  num_v = nrow(in_data)
  max_float <- Inf
  D <- matrix(max_float, nrow = num_v, ncol = num_v)
  for (i in 1:num_v){
    D[i,i] <- 0
    for (j in 1:k){
      v<-knns$nn.index[i,j] 
      D[i,v]<-knns$nn.dist[i,j]
      D[v,i]<-knns$nn.dist[i,j]
    }
  }
