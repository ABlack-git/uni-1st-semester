...
{
  n_vars <- ncol(as.data.frame(data[[1]]))
  withinMatrix <- matrix(0, nrow = n_vars, ncol = n_vars)
  for (i in 1:n) {
    x_c <- as.matrix(as.data.frame(data[[i]]))
    meanVec <- as.matrix(colMeans(x_c))
    for (n_row in 1:nrow(x_c)){
      vec <- as.matrix(x_c[n_row, ]) - meanVec
      withinMatrix<- withinMatrix + vec %*% t(vec)
    }
  }
  return(withinMatrix)
}
...
{
  meanOverall <- as.matrix(meanOverall)
  n_vars <- ncol(as.data.frame(data[[1]]))
  betweenMatrix <- matrix(0, nrow = n_vars, ncol = n_vars)
  for (i in 1:n) {
    x_c <- as.matrix(as.data.frame(data[[i]]))
    vec <- as.matrix(colMeans(x_c)) - meanOverall
    betweenMatrix <- betweenMatrix + nrow(x_c)*(vec%*%t(vec))
  }
...
Classify <- function(newData, eigenVectors, labels, centroids){
  ...
  {
    groupMeanMat <- matrix(rep(as.matrix(centroids[c,]),nrow(newData)), nrow = nrow(newData), byrow = TRUE)
    classification[,c] <- as.matrix(0.5*rowSums((y - groupMeanMat)^2) - log(prior[c]))
  }
...
LDA <- function(mydata, labels){
  ...
  betweenScatterMatrix <- ComputeBetweenScatter(splittedData, n, overallMeans)
...
centroids <- ComputeCentroids(as.matrix(mydata) %*% eigenLDA[,1:(length(levels(labels))-1)],labels = labels)
...
From the graphs above we can clearly see that LDA separates observation by categories way better than PCA. This is due to the fact that LDA and PCA optimize different quantities. LDA's primary task is to maximize the separability of classes, while PCA maximizes vairance of the data. 