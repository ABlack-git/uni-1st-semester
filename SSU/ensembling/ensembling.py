...
class Dataset(object):
    ...
    def __init__(self, df, ix=None):
        ...
        self.ix = np.arange(len(self.data[0]),
            dtype=np.int64) if ix is None else ix
    ...
    def to_dict(self):
        ...
        """
        return [{c: self.data[self.cdict[c]][i] for c in self.columns} for i in
            self.ix]
    ...
    def modify_col(self, cname, d):
        ...
        :return: new Dataset
        """
        assert len(self.ix) == len(
            self.data[0]), 'works for unfiltered rows, only'
...
def generate_boston_housing():
    ...
    data_housing_train, data_housing_test = train_test_split(df, test_size=0.3,
        random_state=1)
...
class RegressionTree(object):
    def __init__(self, data, tattr, xattrs=None, max_depth=5,
        ...
        """
        self.xattrs = [c for c in data.columns if
            c != tattr] if xattrs is None else xattrs
        self.tattr = tattr
        self.max_features = int(np.ceil(max_features(len(self.xattrs))))
        self.rng = rng
        self.root = self.build_tree(data, self.impurity(data),
            max_depth=max_depth)
    ...
    def build_tree(self, data, impurity, max_depth):
        if max_depth > 0:
            ...
            for xattr in xattrs:
                vals = np.unique(
                    data[xattr])  # get all unique values of the attribute
                ...
                for val in vals:
                    ...
                    if split_impurity < best_impurity and len(
                            data_l) > 0 and len(data_r) > 0:
            ...
            if best_impurity < impurity:  # splitting reduces the impurity, choose best possible split
                return DecisionNode(best_xattr, best_val,
                    self.build_tree(best_data_l, best_impurity_l,
                        max_depth - 1),
                    self.build_tree(best_data_r, best_impurity_r,
                        max_depth - 1))
...
def generate_plot(ds_train, ds_test, tattr,
    ...
    for i, val in enumerate(iterate_values):
        st = time()
        ...
        if callable(val):
            iter_val = iterate_labels[i]
        else:
            iter_val = val
        print(
            '{}: {} = {} finished in {:5.2f}s'.format(title, iterate_over,
                iter_val,
                time() - st))
    ...
    if bayes_rmse is not None:
        plt.plot([0, iterate_coords[-1]], [bayes_rmse, bayes_rmse], '-.',
            label='$h^{*}(x)$')
...
def experiment_tree_sin(show=True):
    ...
    plt.savefig('regression_tree_sin.png')
    if show: plt.show()
...
def experiment_tree_housing(show=False):
    ...
    generate_plot(
        data_housing_train, data_housing_test, tattr='medv',
        model_cls=RegressionTree,
        iterate_over='max_depth', iterate_values=range(15),
    ...
    plt.savefig('regression_tree_housing.png')
    if show: plt.show()
